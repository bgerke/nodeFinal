...
if(fs.existsSync("db.json")) {
	var storedUsers = fs.readFileSync("db.json");
	if(!storedUsers) {
		var users = [];
	} else {
...
app.post('/register', function(req, res) {
  if(req.body.username && req.body.password && req.body.email) {
    getHash(req.body.password, function(err, hash) {
	  users.push({username:req.body.username, password:hash, email:req.body.email, wins:0, losses:0, draws:0});
	  
	  res.redirect('/login');
	  
	  fs.writeFile("db.json", JSON.stringify(users), function(err) {
		if(err) return console.error("Error saving db.json.", err);
		console.log("db.json successfully saved.");
	  });
	  
	});
  } else {
    req.flash('error', 'Didn\'t register right!');
